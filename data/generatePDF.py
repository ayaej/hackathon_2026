...
def generate_pdfs(input_file=config.DATASET_JSON, output_dir=config.PDF_DIR, limit=5):
    ...
    for i in range(actual_limit):
        ...
        if(random.random()>.25) : header_client = bold_1 + random.choice(["Client : ", "Nom du client : "]) + random.choice(["<br/>", ""]) + bold_2 + header_client
        if(random.random()>.25) : header_creancier = bold_1 + random.choice(["À : ", "Créancier : ", "Vendeur : ", "Expéditeur : ", "Émetteur : "]) + random.choice(["<br/>", ""]) + bold_2 + header_creancier
        ...
        format_img = random.choice(['jpeg','png','pdf'])
        image_path = f"{os.path.splitext(pdf_path)[0]}_image.{format_img}"
