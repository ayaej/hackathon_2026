...
export default function DocumentList({ documents, onDelete, loading, selectedIds = [], onToggleSelect }) {
  if (loading) {
    return (
    ...
    <div className="divide-y divide-white/5">
      {documents.map((doc) => {
        ...
        return (
          <div
            key={doc._id}
            className={`flex items-center gap-3 py-3 px-2 group rounded-xl transition-colors ${
              isSelected ? 'bg-indigo-500/8 border border-indigo-500/20' : 'hover:bg-white/5'
            }`}
          >
            {onToggleSelect && (
              <button
                onClick={() => onToggleSelect(doc._id)}
            ...
            <a
              ...
              title="Télécharger"
              className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-lg text-white/30 hover:text-indigo-400 hover:bg-indigo-500/10"
