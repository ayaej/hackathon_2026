...
from src.utils.date_checker import verifier_expiration, standardiser_date
...
def test_expiration_rules():
    ...
    attest_recente = (aujourd_hui - relativedelta(months=4)).strftime("%d/%m/%Y")
    res = verifier_expiration(attest_recente, None, "attestation")
    assert res["expire"] is False

    # Attestation ancienne (plus de 6 mois) -> Expire
    attest_ancienne = (aujourd_hui - relativedelta(months=7)).strftime("%d/%m/%Y")
