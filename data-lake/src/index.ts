...
app.get("/api/raw/:documentId", async (req: Request, res: Response) => {
  try {
    const metadata = await rawZoneService.getDocumentMetadata(
...
app.get(
  "/api/raw/:documentId/download",
  async (req: Request, res: Response) => {
    try {
      const { metadata, stream } = await rawZoneService.getDocument(
        req.params.documentId as string,
      );
...
app.post("/api/clean", async (req: Request, res: Response) => {
  try {
    const { documentId, rawDocumentId, extractedText, ocrEngine, options } =
      req.body;
...
app.get("/api/clean/:documentId", async (req: Request, res: Response) => {
  try {
    const document = await cleanZoneService.getCleanDocument(
      req.params.documentId as string,
    );
    if (!document) {
      return res.status(404).json({ error: "Document non trouvé" });
...
app.get(
  "/api/clean/by-raw/:rawDocumentId",
  async (req: Request, res: Response) => {
    try {
      const document = await cleanZoneService.getCleanDocumentByRawId(
        req.params.rawDocumentId as string,
      );
      if (!document) {
        return res.status(404).json({ error: "Document non trouvé" });
      }
...
app.put("/api/clean/:documentId", async (req: Request, res: Response) => {
  try {
    ...
    await cleanZoneService.updateExtractedText(
      req.params.documentId as string,
      extractedText,
      ocrConfidence,
    );
...
app.get("/api/curated/:documentId", async (req: Request, res: Response) => {
  try {
    const document = await curatedZoneService.getCuratedDocument(
      req.params.documentId as string,
    );
    if (!document) {
      return res.status(404).json({ error: "Document non trouvé" });
...
// Search by SIRET
app.get(
  "/api/curated/search/siret/:siret",
  async (req: Request, res: Response) => {
    try {
      const documents = await curatedZoneService.searchBySiret(
        req.params.siret as string,
      );
      res.json(documents);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
...
// Search by SIREN
app.get(
  "/api/curated/search/siren/:siren",
  async (req: Request, res: Response) => {
    try {
      const documents = await curatedZoneService.searchBySiren(
        req.params.siren as string,
      );
      res.json(documents);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
...
app.get(
  "/api/curated/check-inconsistencies/:siret",
  async (req: Request, res: Response) => {
    try {
      const result = await curatedZoneService.checkCompanyInconsistencies(
        req.params.siret as string,
      );
      res.json(result);
    } catch (error: any) {
...
// Update status
app.patch(
  "/api/curated/:documentId/status",
  async (req: Request, res: Response) => {
    try {
      const { status, validationResults } = req.body;
      await curatedZoneService.updateStatus(
        req.params.documentId as string,
        status,
        validationResults,
      );
...
app.patch(
  "/api/curated/:documentId/data",
  async (req: Request, res: Response) => {
    try {
      const { extractedData } = req.body;
      await curatedZoneService.updateExtractedData(
        req.params.documentId as string,
        extractedData,
      );
...
app.post(
  "/api/curated/:documentId/validation",
  async (req: Request, res: Response) => {
    try {
      const validationResult = req.body;
      await curatedZoneService.addValidationResult(
        req.params.documentId as string,
        validationResult,
      );
...
app.delete("/api/curated/:documentId", async (req: Request, res: Response) => {
  try {
    await curatedZoneService.deleteCuratedDocument(
      req.params.documentId as string,
    );
    res.json({ success: true, message: "Document supprimé" });
  } catch (error: any) {
